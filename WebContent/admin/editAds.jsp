          ...
          <div class="wrapper">
          	...
          	<div class="row mt">
                  ...
                  <div class="form-panel">
                  	  <h4 class="mb"><i class="fa fa-angle-right"></i> Sửa quảng cáo</h4>
                  	  <%
		                  	if(request.getAttribute("objAds") != null){
		    					Advertisements objAds = (Advertisements)request.getAttribute("objAds");
                  	  %>
                      <form id="form" class="form-horizontal style-form" action="<%=request.getContextPath()%>/admin/ads/edit?aid=<%=objAds.getIdAds()%>" enctype="multipart/form-data" method="post">
                          <div class="form-group">
                              <label class="col-sm-2 col-sm-2 control-label">Tên quảng cáo:</label>
                              <div class="col-sm-10">
                                  <input name="tenads" type="text" class="form-control" value="<%=objAds.getNameAds()%>">
                              </div>
                          </div>
                          <div class="form-group">
                              ...
                              <div class="col-sm-10">
								 ...
								 %>
									<option <%if(objAds.getLocation() == item.getIdLocal()) {out.print("selected");} %> value="<%=item.getIdLocal()%>"><%=item.getName() %></option>
								<%}}} %>
								  </select>
                              </div>
                          </div>
                          <div class="form-group">
                              ...
                              <div class="col-sm-10">
                                  <input name="link" type="text" class="form-control" value="<%=objAds.getLink()%>">
                              </div>
                          </div>
                          <div class="form-group">
							 ...
							 %>
									<img style="width: 150px;height: 100px" src="<%=request.getContextPath() %>/files/<%=objAds.getPicture() %>" alt="<%=objAds.getPicture() %>" />
							 <%} %>
							 </div>
						  </div>
