          ...
          <div class="wrapper">
          	...
          	<div class="row mt">
                  ...
                  <div class="form-panel">
				 	  ...
				 	  %>
                      <form id="form" class="form-horizontal style-form" method="post" action="<%=request.getContextPath() %>/admin/cat/edit?cid=<%=objCat.getIdCat() %>" >
                          <div class="form-group">
                              <label class="col-sm-2 col-sm-2 control-label">Tên danh mục:</label>
                              <div class="col-sm-10">
                                  <input type="text" class="form-control" name="tendanhmuc" value="<%if(objCat != null){out.print(objCat.getName());}%>">
                              </div>
                          </div>
                          <div class="form-group">
                                  	...
                                  	if(request.getAttribute("listCat") != null){
										...
										if(listCat != null && listCat.size() > 0){
											for(Category item : listCat){
												if(objCat.getIdCat() != item.getIdCat()){
								 %>
									<option <%if(objCat.getIdParent() == item.getIdCat()) {out.print("selected");} %> value="<%=item.getIdCat()%>"><%if(item.getIdParent() > 0){out.print("|--"+item.getName());}else{out.print(item.getName());} %></option>
								<%}}}} %>
								  </select>
                              </div>
                          </div>
                          <div class="form-group">
                              <label class="col-sm-2 col-sm-2 control-label">Thứ tự hiển thị:</label>
                              <div class="col-sm-10">
                                  <select class="form-control" name="indexDisplay">
                                  <%
                                  	if(request.getAttribute("listDisplayIndex") != null){
                                  		@SuppressWarnings("unchecked") 
										ArrayList<Category> listDisplayIndex = (ArrayList<Category>) request.getAttribute("listDisplayIndex");
										if(listDisplayIndex != null && listDisplayIndex.size() > 0){
											for(Category item : listDisplayIndex){
								  %>
									<option <%if(objCat.getDisplayIndex() == item.getDisplayIndex()) {out.print("selected");} %> value="<%=item.getDisplayIndex()%>"><%=item.getDisplayIndex() %></option>
								  <%}}} %>
								  </select>
                              </div>
                          </div>
